...
Elm.Color.Convert.make = function (_elm) {
   ...
   var xyzToColor = function (_p0) {
      var _p1 = _p0;
      var c = function (ch) {    return _U.cmp(ch,3.1308e-3) > 0 ? 1.055 * Math.pow(ch,1 / 2.4) - 5.5e-2 : 12.92 * ch;};
      var z$ = _p1.z / 100;
      var y$ = _p1.y / 100;
      var x$ = _p1.x / 100;
      return A3($Color.rgb,
   ...
   var labToXyz = function (_p2) {
      var _p3 = _p2;
      var y = (_p3.l + 16) / 116;
      var c = function (ch) {    var ch$ = ch * ch * ch;return _U.cmp(ch$,8.856e-3) > 0 ? ch$ : (ch - 16 / 116) / 7.787;};
      return {y: c(y) * 100,x: c(y + _p3.a / 500) * 95.047,z: c(y - _p3.b / 200) * 108.883};
   };
   var labToColor = function (lab) {    return xyzToColor(labToXyz(lab));};
   var xyzToLab = function (_p4) {
      var _p5 = _p4;
      var c = function (ch) {    return _U.cmp(ch,8.856e-3) > 0 ? Math.pow(ch,1 / 3) : 7.787 * ch + 16 / 116;};
   ...
   var colorToXyz = function (cl) {
      ...
      var c = function (ch) {
         ...
         var ch$$ = _U.cmp(ch$,4.045e-2) > 0 ? Math.pow((ch$ + 5.5e-2) / 1.055,2.4) : ch$ / 12.92;
         return ch$$ * 100;
      };
      var r = c(red);
      var g = c(green);
   ...
   var colorToLab = function (cl) {    return xyzToLab(colorToXyz(cl));};
   var toRadix = function (n) {
      var getChr = function (c) {    return _U.cmp(c,10) < 0 ? $Basics.toString(c) : $String.fromChar($Char.fromCode(87 + c));};
      return _U.cmp(n,16) < 0 ? getChr(n) : A2($Basics._op["++"],toRadix(n / 16 | 0),getChr(A2($Basics._op["%"],n,16)));
   };
   var toHex = function (n) {    var hex = toRadix(n);return _U.eq($String.length(hex),1) ? A2($Basics._op["++"],"0",hex) : hex;};
   var colorToHex = function (cl) {
      ...
      var blue = _p7.blue;
      var alpha = _p7.alpha;
      return A2($Basics._op["++"],"#",A2($Basics._op["++"],toHex(red),A2($Basics._op["++"],toHex(green),toHex(blue))));
   };
   var hexToColor = function (c) {
      ...
      A3($Regex.find,$Regex.All,$Regex.regex("^#?([a-f\\d]{2})([a-f\\d]{2})([a-f\\d]{2})$"),$String.toLower(c))));
      var _p8 = r;
      if (_p8.ctor === "Just") {
            var v = $Array.fromList(A2($List.filterMap,
            $Basics.identity,
            A2($List.map,$Result.toMaybe,A2($List.map,$ParseInt.parseIntHex,A2($List.filterMap,$Basics.identity,_p8._0)))));
   ...
   var cssColorString = F2(function (kind,values) {
      return A2($Basics._op["++"],kind,A2($Basics._op["++"],"(",A2($Basics._op["++"],A2($String.join,", ",values),")")));
   });
   var toPercentString = function (h) {    return A2($Basics._op["++"],$Basics.toString($Basics.round(h * 100)),"%");};
   var hueToString = function (h) {    return $Basics.toString($Basics.round(h * 180 / $Basics.pi));};
   var colorToCssHsla = function (cl) {
      ...
      return A2(cssColorString,"hsla",_U.list([hueToString(hue),toPercentString(saturation),toPercentString(lightness),$Basics.toString(alpha)]));
   };
   var colorToCssHsl = function (cl) {
      ...
      return A2(cssColorString,"hsl",_U.list([hueToString(hue),toPercentString(saturation),toPercentString(lightness)]));
   };
   var colorToCssRgba = function (cl) {
      ...
      return A2(cssColorString,"rgba",_U.list([$Basics.toString(red),$Basics.toString(green),$Basics.toString(blue),$Basics.toString(alpha)]));
   };
   var colorToCssRgb = function (cl) {
      ...
      return A2(cssColorString,"rgb",_U.list([$Basics.toString(red),$Basics.toString(green),$Basics.toString(blue)]));
   };
...
Elm.Main.make = function (_elm) {
   ...
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var ticking = $Time.every($Time.inMilliseconds(50));
   var mouseClicks = $Time.timestamp(A2($Signal.sampleOn,$Mouse.clicks,$Mouse.position));
   var drawBubble = function (bubble) {
      return A2($Svg.circle,
      _U.list([$Svg$Attributes.cx($Basics.toString(bubble.x))
              ,$Svg$Attributes.cy($Basics.toString(bubble.y))
              ,$Svg$Attributes.r($Basics.toString(bubble.r))
              ,$Svg$Attributes.fill($Color$Convert.colorToHex(bubble.c))
              ,$Svg$Attributes.opacity($Basics.toString(bubble.o))]),
      _U.list([]));
   };
   var view = F2(function (address,model) {
      return A2($Svg.svg,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: "100%"},{ctor: "_Tuple2",_0: "height",_1: "100vh"}]))]),
      A2($List.map,drawBubble,model));
   });
   var targetRadius = 250;
   var expandBubble = function (b) {    return _U.update(b,{r: b.r + 1,o: b.o - 1.0 / $Basics.toFloat(targetRadius)});};
   var extant = function (bubble) {    return _U.cmp(bubble.r,targetRadius) < 1;};
   var init = {ctor: "_Tuple2",_0: _U.list([]),_1: $Effects.none};
   var Incoming = function (a) {    return {ctor: "Incoming",_0: a};};
   var MsgEmitted = function (a) {    return {ctor: "MsgEmitted",_0: a};};
   var Tick = function (a) {    return {ctor: "Tick",_0: a};};
   var Click = function (a) {    return {ctor: "Click",_0: a};};
   var driver = A2($Signal.merge,A2($Signal.map,Click,mouseClicks),A2($Signal.map,Tick,ticking));
   var parseBubblePosition = function (s) {
      var decoder = A4($Json$Decode.tuple3,
      F3(function (v0,v1,v2) {    return {ctor: "_Tuple3",_0: v0,_1: v1,_2: v2};}),
      $Json$Decode.$int,
      $Json$Decode.$int,
      $Json$Decode.$int);
      return A2($Json$Decode.decodeString,decoder,s);
   };
   var generateColor = function (t) {
      var seed = $Random.initialSeed(t);
      var intGenerator = A2($Random.$int,0,255);
      var _p0 = A2($Random.generate,intGenerator,seed);
      var r = _p0._0;
      var s = _p0._1;
      var _p1 = A2($Random.generate,intGenerator,s);
      var g = _p1._0;
      var s$ = _p1._1;
      var _p2 = A2($Random.generate,intGenerator,s$);
      var b = _p2._0;
      return A3($Color.rgb,r,b,g);
   };
   var initBubble = F3(function (t,x,y) {    return {x: x,y: y,r: 0,o: 1.0,c: generateColor(t)};});
   var Bubble = F5(function (a,b,c,d,e) {    return {x: a,y: b,r: c,o: d,c: e};});
   var newIncomingBubble = Elm.Native.Port.make(_elm).inboundSignal("newIncomingBubble",
   "Main.BubblePos",
   function (v) {
      return typeof v === "object" && "t" in v && "x" in v && "y" in v ? {_: {}
                                                                         ,t: typeof v.t === "number" && isFinite(v.t) && Math.floor(v.t) === v.t ? v.t : _U.badPort("an integer",
                                                                         v.t)
                                                                         ...
                                                                         v.y)} : _U.badPort("an object with fields `t`, `x`, `y`",v);
   });
   var incomingBubbles = A2($Signal.map,Incoming,newIncomingBubble);
   var mkBubblePos = function (_p3) {    var _p4 = _p3;return {t: _p4._0,x: _p4._1,y: _p4._2};};
   var BubblePos = F3(function (a,b,c) {    return {t: a,x: b,y: c};});
   var mailbox = $Signal.mailbox({ctor: "_Tuple3",_0: 0,_1: 0,_2: 0});
   var newBubble = Elm.Native.Port.make(_elm).outboundSignal("newBubble",
   function (v) {
      return {t: v.t,x: v.x,y: v.y};
   },
   A2($Signal.map,mkBubblePos,mailbox.signal));
   var update = F2(function (action,model) {
      var _p5 = action;
      switch (_p5.ctor)
      {case "Click": var msg = A2($Signal.send,mailbox.address,{ctor: "_Tuple3",_0: $Basics.truncate(_p5._0._0),_1: _p5._0._1._0,_2: _p5._0._1._1});
           return {ctor: "_Tuple2",_0: model,_1: $Effects.task(A2($Task.map,MsgEmitted,msg))};
         case "Tick": var newModel = A2($List.filter,extant,model);
           return {ctor: "_Tuple2",_0: A2($List.map,expandBubble,newModel),_1: $Effects.none};
         case "MsgEmitted": return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
         default: return {ctor: "_Tuple2",_0: A2($List._op["::"],A3(initBubble,_p5._0.t,_p5._0.x,_p5._0.y),model),_1: $Effects.none};}
   });
   var app = $StartApp.start({init: init,update: update,view: view,inputs: _U.list([driver,incomingBubbles])});
   var main = app.html;
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   return _elm.Main.values = {_op: _op
                             ,mailbox: mailbox
                             ,BubblePos: BubblePos
                             ,mkBubblePos: mkBubblePos
                             ,incomingBubbles: incomingBubbles
                             ,Bubble: Bubble
                             ,initBubble: initBubble
                             ,generateColor: generateColor
                             ,parseBubblePosition: parseBubblePosition
                             ,Click: Click
                             ,Tick: Tick
                             ,MsgEmitted: MsgEmitted
                             ,Incoming: Incoming
                             ,init: init
                             ,targetRadius: targetRadius
                             ,expandBubble: expandBubble
                             ,extant: extant
                             ,update: update
